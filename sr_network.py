...
class SR_NETWORK(object):
    ...
    def generator(self, x, r, is_training=False, reuse=False):
        if is_training:
            N, h, w, C = tf.shape(input=x)[0], x.get_shape()[1], x.get_shape()[2], x.get_shape()[3]
        else:
            N, h, w, C = tf.shape(input=x)[0], tf.shape(input=x)[1], tf.shape(input=x)[2], x.get_shape()[3]
        ...
        with tf.compat.v1.variable_scope('generator', reuse=reuse):
            ...
            for i, r_i in enumerate(r):
                ...
                with tf.compat.v1.variable_scope('deconv{}'.format(i+3)):
                    ...
                    x = deconv_layer_2d(x, [k, k, C_out, C_in], output_shape, stride, k)
                    x = tf.compat.v1.depth_to_space(input=x, block_size=r_i)
                    x = tf.nn.relu(x)
    ...
    def compute_losses(self, x_HR, x_SR, d_HR, d_SR, alpha_advers=0.001, isGAN=False):
        ...
        content_loss = tf.reduce_mean(input_tensor=(x_HR - x_SR)**2, axis=[1, 2, 3])
        ...
        if isGAN:
            ...
            advers_perf = [tf.reduce_mean(input_tensor=tf.cast(tf.sigmoid(d_HR) > 0.5, tf.float32)), # % true positive
                           tf.reduce_mean(input_tensor=tf.cast(tf.sigmoid(d_SR) < 0.5, tf.float32)), # % true negative
                           tf.reduce_mean(input_tensor=tf.cast(tf.sigmoid(d_SR) > 0.5, tf.float32)), # % false positive
                           tf.reduce_mean(input_tensor=tf.cast(tf.sigmoid(d_HR) < 0.5, tf.float32))] # % false negative

            g_loss = tf.reduce_mean(input_tensor=content_loss) + alpha_advers*tf.reduce_mean(input_tensor=g_advers_loss)
            d_loss = tf.reduce_mean(input_tensor=d_advers_loss)
